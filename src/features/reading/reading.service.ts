...
async function findActiveSession(userId: Types.ObjectId, storyId: Types.ObjectId) {
  return StoryReadSession.findOne({ userId, storyId, finishedAt: { $exists: false } }).sort({ startedAt: -1 })
}
...
export async function openOrPage(
  ...
  if (!session) {
    ...
    const anySessionBefore = await StoryReadSession.exists({ userId, storyId: story._id })
    if (!anySessionBefore) {
  ...
  if (page >= pages - 1 && !session.completed) {
    ...
    session.lastEventAt = new Date()
    await session.save()
    await Story.updateOne({ _id: story._id }, { $inc: { 'stats.completedCount': 1, 'stats.conversions.completed': 1 } })
  }
}
...
export async function dropActiveSession(
  ctx: MyContext,
  storyId: string
) {
  const u = ctx.state.user
