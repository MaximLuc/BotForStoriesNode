...
import { getOrCreateDraft, resetPending, setField, setEndingTitle, setEndingText } from './draft.service'
...
async function updateMenu(ctx: MyContext, text: string, inline?: any) {
  const kb = inline?.reply_markup ? inline : (inline ? { reply_markup: inline } : undefined)
  try {
...
export function registerDraftFinishHandlers(bot: Telegraf<MyContext>) {
  ...
  bot.action('draft:finish_input', async (ctx) => {
    ...
    if (!fin) {
      const payload = await renderAddStoryTextScreen(ctx)
      return updateMenu(ctx, payload.text + '\n\n<i>Нет текста для сохранения.</i>', payload.inline)
    }
