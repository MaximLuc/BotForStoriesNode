...
import { respond } from "../../app/ui/respond";
...
export function registerFileImportActions(bot: Telegraf<MyContext>) {
  bot.action("admin:import_file", async (ctx) => {
    if (!ctx.state.user || !isAdmin(ctx.state.user)) {
      ...
      return respond(ctx, "Доступ только для админа.", {
        inline: Markup.inlineKeyboard([
          [Markup.button.callback("↩︎ Назад", "main")],
        ]),
        parseMode: "HTML",
      });
    }
    ...
    return respond(ctx, `📨 Загрузка файла для импорта\n\n${HELP()}`, {
      inline: Markup.inlineKeyboard([
