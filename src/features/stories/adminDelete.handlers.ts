...
async function showPhotoOneWindow(
  ...
  const kb = inline?.reply_markup ? inline : (inline ? { reply_markup: inline.reply_markup ?? (inline as any) } : undefined)
  const src: any =
  ...
  if (isPhoto) {
    try {
      await ctx.editMessageCaption(caption, { parse_mode: 'Markdown', ...(kb ?? {}) })
      return
    } catch {
      try { await ctx.telegram.deleteMessage(src.chat.id, src.message_id) } catch {}
      const sent = await ctx.replyWithPhoto(fileId, { caption, parse_mode: 'Markdown', ...(kb ?? {}) })
...
async function renderDeleteList(ctx: MyContext, page = 0) {
  ...
  const lines = items
    .map(s => `• ${s.title} (${(s.endings ?? []).length})`)
    .join('\n') || '_Пока пусто_'
  ...
  const kb = Markup.inlineKeyboard([
    ...grid,
    [
      Markup.button.callback('⟨ Назад', `admin:delete_list:p:${Math.max(0, page - 1)}`),
      Markup.button.callback(`Стр. ${page + 1}/${pages}`, 'noop'),
      Markup.button.callback('Вперёд ⟩', `admin:delete_list:p:${Math.min(pages - 1, page + 1)}`),
    ],
...
async function renderStoryCard(ctx: MyContext, storyId: string, backPage: number) {
  const s = await Story.findById(storyId).lean<StoryLean>()
  if (!s) {
    return safeEdit(
      ctx,
      'История не найдена.',
      Markup.inlineKeyboard([[Markup.button.callback('↩︎ Назад к списку', `admin:delete_list:p:${backPage}`)]])
    )
  }

  const endingsCount = (s.endings ?? []).length
  const endingsNames =
    (s.endings ?? []).map((e, i) => `#${i + 1}: ${e.title || '—'}`).join('; ') || '—'
...
Выберите действие ниже.`
  ...
  const kb = Markup.inlineKeyboard([
    [Markup.button.callback('🗑 Удалить историю', `admin:ask_delete:${s._id}:p:${backPage}`)],
    [Markup.button.callback('↩︎ Назад к списку', `admin:delete_list:p:${backPage}`)],
    [Markup.button.callback('↩︎ В админ-меню', 'admin')],
  ])
...
export function registerAdminDeleteHandlers(bot: Telegraf<MyContext>) {
  bot.action('admin:delete_list', async (ctx) => {
    ...
    if (!ctx.state.user || !isAdmin(ctx.state.user)) {
      return safeEdit(ctx, 'Доступ только для админа.', Markup.inlineKeyboard([[Markup.button.callback('↩︎ Назад', 'main')]]))
    }
  ...
  bot.action(/^admin:ask_delete:([a-fA-F0-9]{24}):p:(\d+)$/, async (ctx) => {
    ...
    const kb = Markup.inlineKeyboard([
      [
        Markup.button.callback('❌ Да, удалить', `admin:confirm_delete:${id}:p:${backPage}`),
        Markup.button.callback('↩︎ Назад', `admin:delete:open:${id}:p:${backPage}`),
      ],
    ])
    return safeEdit(ctx, `⚠️ Точно удалить *${s.title}*? Это действие необратимо.`, kb)
  })

  bot.action(/^admin:confirm_delete:([a-fA-F0-9]{24}):p:(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery()
    const id = String(ctx.match[1])
    const backPage = Number(ctx.match[2])
    ...
    try {
      ...
      await safeEdit(ctx, `✅ История *${s.title}* удалена.`, Markup.inlineKeyboard([
        [Markup.button.callback('↩︎ К списку', `admin:delete_list:p:${backPage}`)]
      ]))
      ...
      return safeEdit(
        ...
        Markup.inlineKeyboard([[Markup.button.callback('↩︎ К списку', `admin:delete_list:p:${backPage}`)]])
      )
    }
  })
