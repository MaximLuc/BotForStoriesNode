...
export async function bufferToText(
  buf: Buffer,
  mime: string,
  filename?: string
): Promise<string> {
...
export function parseStoryFromText(raw: string): ParsedStory {
  ...
  let e1: EndingParsed = {};
  b = captureBlock(text, tagRe(TAGS.E1T), nextAll);
  if (b.value) e1.title = b.value;
  text = b.rest;
  b = captureBlock(text, tagRe(TAGS.E1X), nextAll);
  if (b.value) e1.text = b.value;
  text = b.rest;
  if (e1.title || e1.text) res.endings.push(e1);

  let e2: EndingParsed = {};
  b = captureBlock(text, tagRe(TAGS.E2T), nextAll);
  if (b.value) e2.title = b.value;
  text = b.rest;
  b = captureBlock(text, tagRe(TAGS.E2X), nextAll);
  if (b.value) e2.text = b.value;
  text = b.rest;
  if (e2.title || e2.text) res.endings.push(e2);

  let e3: EndingParsed = {};
  b = captureBlock(text, tagRe(TAGS.E3T), nextAll);
  if (b.value) e3.title = b.value;
  text = b.rest;
  b = captureBlock(text, tagRe(TAGS.E3X), nextAll);
  if (b.value) e3.text = b.value;
  text = b.rest;
